...
Keskkonnad on sisuliselt textmode käsud, mille süntaks on keerulisem kui textmode käskudel, kuid mille abil on võimalik defineerida \LaTeX \ keskkondi, mida textmode käskude abil teha ei saa või on mõttetult tülikas teha. Keskkonnad töötavad peamiselt nendes defineeritud käskude abil, mille defineerimine on täpselt samasugune, nagu mathmode käskude puhul (vt. alapeatükki 2.2). Siinkohal on oluline märkida, et keskkondade sees kasutatakse ühe käsu mitme argumendi eraldamiseks kolme tühikut!
...
\begin{enumerate}
\item \tcbox[before=, after=, tcbox raise base]{keskkonnaNimi} -- See on tekst, mida otsitakse lähtefailis ning mille nägemisel on selge, et siit algab uus keskkond. Kõik sellel järgnev tekst loetakse keskkonna osaks.
...
\begin{enumerate}
	...
	\item \tcbox[before=, after=, tcbox raise base]{multiline} -- Selle lipu olemasolu tähendab, et keskkond lubab kirjutada teksti mitmele reale, ilma et oleks tarvis välja kutsuda igal real vastavat käsku. Seega on multiline lipu olemasolul tarvis välja kutsuda vastav käsk igal real, millel soovitakse, et see käsk rea ette pannakse. Kui rea ees tuntud käsku ei ole, siis eeldatakse, et see rida on jätk viimasena välja kutsutud käsule. Kui multiline lipp puudub, siis on kaks võimalust: kui keskkonnas on defineeritud kuni üks käsk, siis ei ole tarvis iga rea ette käsu nime kirjutada, see pannakse sinna automaatselt. Kui on aga rohkem kui üks käsk defineeritud, siis peab \textbf{igal real} välja kutsuma mõne käsu, sest multiline lipp puudub.
	...
	\begin{tcolorbox}
	\begin{verbatim}
	mat2 [end:{--}] -> \begin{pmatrix} #content \end{pmatrix} | (r(el1)(el2) -> el1 & el2 \\)
	\end{verbatim}
	\end{tcolorbox}
	Et multiline lippu pole ja et on defineeritud ainult üks käsk \code{r}, siis ei pea me ühelgil real käsku välja kutsuma, seda tehakse meie eest. Kirjutades lähtekoodis:
	\begin{tcolorbox}
	\begin{verbatim}
	mat2
	2   5
...
\item \tcbox[before=, after=, tcbox raise base]{\textbackslash begin\{keskkonna algus\} \#content \textbackslash end\{keskkonna lõpp\}} -- Käskude begin ja end sisse tuleb panna keskkonna nimi nii, nagu see on \LaTeX is nimetatud. Enamasti (alati?) on keskkonna lõpu ja alguse nimed samad.
